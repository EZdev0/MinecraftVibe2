...
public class Gameplay {
    ...
    public class ActiveFire {
        ...
        public void init(int x, int y, int z) { this.x = x; this.y = y; this.z = z; this.life = 10.0f + random.nextFloat() * 10f; }
    }
    ...
    public void update(float dt, WorldLogic world) {
        ...
        for (int i = activeFires.size() - 1; i >= 0; i--) {
            ...
            if (random.nextFloat() < 0.2f) {
                fireParticles.add(obtainParticle(f.x + random.nextFloat(), f.y + random.nextFloat(), f.z + random.nextFloat(), 0, 1.0f + random.nextFloat(), 0, 0.5f + random.nextFloat(), Blocks.FIRE));
            }
        }
