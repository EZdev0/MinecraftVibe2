        ...
        for (int x = 0; x < 16; x++) {
            for (int z = 0; z < 16; z++) {
                ...
                for (int y = 0; y <= height; y++) {
                    ...
                    float worm = Noise.simplex3(gx * 0.035f, y * 0.035f, gz * 0.035f);
                    float thickness = 0.02f + ((128f - y) / 128f) * 0.04f;
                    boolean isTunnel = Math.abs(worm - 0.5f) < thickness;
                    float room = Noise.simplex3(gx * 0.015f, y * 0.015f, gz * 0.015f);
                    if (! (y > height - 5) && (isTunnel || room > 0.78f)) {
                        blocks[x][y][z] = 0;
                    } else {
